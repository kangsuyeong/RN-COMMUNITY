...
function PostWriteFooter({}: PostWriteFooterProps) {
  ...
  const [imaageUris] = useWatch({ control, name: ['imageUris'] });

  const addImageUris = (uris: string[]) => {
    if (imaageUris.length + uris.length > 5) {
      Alert.alert('이미지 개수 초과', '추가 가능한 이미지는 최대 5개 입니다.');
      return;
    }
    setValue('imageUris', [...imaageUris, ...uris.map((uri) => ({ uri }))]);
  };

  const handleOpeImagePick = async () => {
  ...
  return (
    <View style={[styles.container, { paddingBottom: inset.bottom }]}>
      <Pressable style={styles.footerIcon} onPress={handleOpeImagePick}>
